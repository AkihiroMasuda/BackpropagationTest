...
%	ind3 = find((xx(:)>=-3)&(xx(:)<=3));
	...
	for KK_ = 1:N
		for II_ = 1:size(X,2)
			...
			x = X(:,II_);
			%% 最終出力
			[u g1 g2 g3] = calcans(x, w1, w2, w3, h1, h2, h3, @sigmo);
			%%学習データ（正解値）
			b = x(end-2:end); 
			%% 中間→出力層の修正量
			e1 = -[g1; g2; g3; 1]*(u(1)-b(1))*u(1)*(1-u(1));
			e2 = -[g1; g2; g3; 1]*(u(2)-b(2))*u(2)*(1-u(2));
			e3 = -[g1; g2; g3; 1]*(u(3)-b(3))*u(3)*(1-u(3));
%			e = -[g1; g2; g3; 1]*((u-b)*u*(1-u))';
			%% 入力→中間層の修正用
			c1 = x(1:3)*(e1(1)*h1(1)+e2(1)*h2(1)+e3(1)*h3(1))*g1*(1-g1);
			c2 = x(1:3)*(e1(2)*h1(2)+e2(2)*h2(2)+e3(2)*h3(2))*g2*(1-g2);
			c3 = x(1:3)*(e1(3)*h1(3)+e2(3)*h2(3)+e3(3)*h3(3))*g3*(1-g3);
			%% 修正
			w1 = w1 + k*c1;
			w2 = w2 + k*c2;
			w3 = w3 + k*c3;
...
%	ans0 = cu(1) <= 0.5;
	...
	[u] = calcans(X2, w1, w2, w3, h1, h2, h3, @getclass);
	ans1_2 = find(u(1,:)==1);
	ans2_2 = find(u(2,:)==1);
	ans3_2 = find(u(3,:)==1);
...
% ニューラルネットの答えを求める
function [cu cg1 cg2 cg3] = calcans(X, w1, w2, w3, h1, h2, h3, f)
	cg1 = f(w1'*X(1:3,:));
	cg2 = f(w2'*X(1:3,:));
	cg3 = f(w3'*X(1:3,:));
	cu(1,:) = f(h1'*[cg1; cg2; cg3; ones(1,size(X,2))]);
	cu(2,:) = f(h2'*[cg1; cg2; cg3; ones(1,size(X,2))]);
	cu(3,:) = f(h3'*[cg1; cg2; cg3; ones(1,size(X,2))]);
end
